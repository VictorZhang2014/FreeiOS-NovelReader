...
#define NSDateTimeAgoLocalizedStrings(key) [NSDate localizedStringInAppWithKey:key]
#endif


// 跟随系统的语言来读取时间
+ (NSString *)localizedStringWithKey:(NSString *)key {
    NSString *value = NSLocalizedStringFromTableInBundle(key, @"NSDateTimeAgo", [NSBundle bundleWithPath:[[[NSBundle bundleForClass:[DummyClass class]] resourcePath] stringByAppendingPathComponent:@"NSDateTimeAgo.bundle"]], nil);
    return value;
}

// 跟随用户在app内切换的语言来读取时间
+ (NSString *)localizedStringInAppWithKey:(NSString *)key {
    NSString *currentLanguage = [LNLanguageToggle shared].language;
    NSString *datetimeAgoBundlePath = [[NSBundle mainBundle] pathForResource:@"NSDateTimeAgo" ofType:@"bundle"];
    NSString *lprojPath = [[NSBundle bundleWithPath:datetimeAgoBundlePath] pathForResource:currentLanguage ofType:@"lproj"];
    NSBundle *bundle = [NSBundle bundleWithPath:lprojPath];
    if (bundle==nil) return key;
    NSString *value = NSLocalizedStringFromTableInBundle(key, @"NSDateTimeAgo", bundle, nil);
    return value;
}
...
- (NSString *)dateTimeAgoInPrivateMessage
{
    ...
    {
        return [self getTimeWithShowingWeek:NO];
    }
    else if (deltaMinutes < 120)
    {
        return [self getTimeWithShowingWeek:NO];
    ...
    {
        minutes = (int)floor(deltaMinutes/60);
        return [NSString stringWithFormat:@"%@ %@", [self stringFromFormat:@"%%d %@hours ago" withValue:minutes], [self getTimeWithShowingWeek:NO]];
    }
    else if (deltaMinutes < (24 * 60 * 2))
    {
        return [NSString stringWithFormat:@"%@ %@", NSDateTimeAgoLocalizedStrings(@"Yesterday"), [self getTimeWithShowingWeek:NO]];
    }
    else if (deltaMinutes < (24 * 60 * 7))
    {
        return [self getTimeWithShowingWeek:YES];
    }
    else if (deltaMinutes < (24 * 60 * 14))
    {
        return [NSString stringWithFormat:@"%@ %@", NSDateTimeAgoLocalizedStrings(@"Last week"), [self getTimeWithShowingWeek:YES]];
    }
    else if (deltaMinutes < (24 * 60 * 31))
    {
        minutes = (int)floor(deltaMinutes/(60 * 24 * 7));
...
- (NSString *)dateTimeAgoRoughly
{
    ...
    {
        return NSDateTimeAgoLocalizedStrings(@"Minutes ago");
    }
    else if (deltaMinutes < (24 * 60))
    {
        return NSDateTimeAgoLocalizedStrings(@"Hours ago");
    }
    else if (deltaMinutes < (24 * 60 * 2))
    {
        return [NSString stringWithFormat:@"%@ %@", NSDateTimeAgoLocalizedStrings(@"Yesterday"), [self getTimeWithShowingWeek:NO]];
    ...
    {
        return NSDateTimeAgoLocalizedStrings(@"Days ago");
    }
    else if (deltaMinutes < (24 * 60 * 14))
    {
        return NSDateTimeAgoLocalizedStrings(@"Last week");
    ...
    {
        return NSDateTimeAgoLocalizedStrings(@"Months ago");
    }
    else if (deltaMinutes < (24 * 60 * 365.25))
    {
        return NSDateTimeAgoLocalizedStrings(@"Months ago");
...
// 例如：11:11 AM， 06:19 PM
- (NSString *)getTimeWithShowingWeek:(BOOL)isShowedWeek {
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    if (isShowedWeek) {
        formatter.dateFormat = @"EEEE hh:mm a"; // EEEE hh:mm a = Sunday 12:12 PM
    } else {
        formatter.dateFormat = @"hh:mm a"; // hh:mm a = 12:12 PM
    }
    return [formatter stringFromDate:self];
}
